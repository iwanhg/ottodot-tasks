...
# POST /bookings -> draft, id 6
curl -X POST http://localhost:3000/bookings \
    -H "Content-Type: application/json" \
    -d '{"studentId":6,"trialClassId":1}' 

# POST /bookings/:id/submit -> pending_payment, seat reserved
curl -X POST http://localhost:3000/bookings/6/submit

# POST /bookings/:id/pay -> confirmed
curl -X POST http://localhost:3000/bookings/6/pay \
    -H "Content-Type: application/json" \
    -d '{"success":true}'

# GET /classes/:id/roster -> Finn now included, 2/4 confirmed
curl http://localhost:3000/classes/1/roster
```
...
curl -X POST http://localhost:3000/bookings \
    -H "Content-Type: application/json" \
    -d '{"studentId":6,"trialClassId":2}'

# draft, id 8
curl -X POST http://localhost:3000/bookings \
    -H "Content-Type: application/json" 
    -d '{"studentId":7,"trialClassId":2}'

# pending_payment
curl -X POST http://localhost:3000/bookings/7/submit   
curl -X POST http://localhost:3000/bookings/8/submit
```

**Demo a mock payment** (Finn pays for the booking he just reserved):
```bash
curl -X POST http://localhost:3000/bookings/7/pay \
    -H "Content-Type: application/json" \
    -d '{"success":true}'
```
